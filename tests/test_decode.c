...
TEST("decode: decode(encode(x)) == x") {
    ...
    const char *input = "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod "
    ...
    /* Encode the text. */
    char *encoded;
    passwand_error_t err = encode((const uint8_t*)input, strlen(input), &encoded);
    CU_ASSERT_EQUAL_FATAL(err, PW_OK);
