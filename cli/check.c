...
static const char *get_ssl_error(const SSL *ssl, int ret) {
    switch (SSL_get_error(ssl, ret)) {
        case SSL_ERROR_ZERO_RETURN:      return "SSL_ERROR_ZERO_RETURN (see man SSL_get_error)";
        case SSL_ERROR_WANT_READ:        return "SSL_ERROR_WANT_READ (see man SSL_get_error)";
        case SSL_ERROR_WANT_WRITE:       return "SSL_ERROR_WANT_WRITE (see man SSL_get_error)";
        case SSL_ERROR_WANT_CONNECT:     return "SSL_ERROR_WANT_CONNECT (see man SSL_get_error)";
        case SSL_ERROR_WANT_ACCEPT:      return "SSL_ERROR_WANT_ACCEPT (see man SSL_get_error)";
        case SSL_ERROR_WANT_X509_LOOKUP: return "SSL_ERROR_WANT_X509_LOOKUP (see man SSL_get_error)";
        case SSL_ERROR_SYSCALL:          return "SSL_ERROR_SYSCALL (see man SSL_get_error)";
        case SSL_ERROR_SSL:              return "SSL_ERROR_SSL (see man SSL_get_error)";
...
static char *hibp_data(const char *hex, const char **error) {
    ...
    if (fd < 0) {
        ...
        if (error != NULL)
            *error = "failed to find a reachable IP address for api.pwnedpasswords.com";
        return NULL;
    }
    ...
    /* send the request */
    {
        char buffer[sizeof("GET /range/XXXXX HTTP/1.0\r\n"
...
static void loop_body(const char *space, const char *key, const char *value) {
    ...
    if (in_dictionary(value)) {
        ...
        if (data == NULL) {
            print("%s/%s: skipped (%s)\n", space, key, error == NULL ? "unknown cause" : error);
            return;
        }
